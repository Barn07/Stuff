...
/// ultra short things only for private usage ;)
    ...
    #pragma warning(disable:4244) // Konvertierung 'Konvertierung' von 'Typ1' nach 'Typ2', Datenverlust möglich
    ...
    #define FORALL( CONTAINER)  for( auto it = CONTAINER.begin(); it != CONTAINER.end(); ++it)
...
XXX make it stable for big containers
...
*/
template< typename F, template<class T, class = std::allocator<T> > class container_type >
int binary_search(const container_type<F>& container, const F value) {

    int idx = static_cast<int>(container.size()) / 2;
    int stepsize = idx;
...
*/
template< typename T>
constexpr inline T range_intersection(const T lower1, const T upper1, const T lower2, const T upper2) {
